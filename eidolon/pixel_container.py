...
class PixelContainer(train.Container):
    ...
    def on_prepare(self):
        ...
        """
        # 加载数据集
        train_dataset = loader.load_custom_image_dataset(
            self.config_loader, is_training=True)
        test_dataset = loader.load_custom_image_dataset(
        ...
        if self.config_loader.discriminator != "no":
            self.register_display_loss(["gen_loss", "disc_loss"])
        else:
            self.register_display_loss(["gen_loss"])
