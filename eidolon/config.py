...
default_config = {
    ...
    "pixel2pixel": {
        ...
        "callback_args": {
            "value": ["--decoder=new", "--wm_path=../../WMNetv2/watermark/wm_binary_x64.png","--wm_width=64"],
            "desc": "设置回调函数输入参数，用于初始化自定义的回调参数。目前支持--key=value格式。输入内容为数组，每个元素请用逗号隔开。目前有效参数：--decoder, --attack, --wm_path, --wm_width。注意：v2.0版推荐wm_path选择new"
        },
        "lambda": {
            "value": [1, 1],
